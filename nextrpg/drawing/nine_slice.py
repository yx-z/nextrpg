...
class NineSlice:
    ...
    def stretch(self, size: Size) -> DrawingGroup:
        ...
        center_row = center_row_group.shift(self.top.zero_width)
        ...
        bottom_row = bottom_row_group.shift(
            size.height.zero_width, Anchor.BOTTOM_LEFT
        )

        parts = [top_row, center_row, bottom_row]
        if (debug := config().debug) and (color := debug.draw_group_link_color):
            points = (ORIGIN, size.height.zero_width.coordinate)
            vertical_line = PolylineDrawing(points, color).drawing
            parts += [
                vertical_line.shift(self.left.zero_height),
                vertical_line.shift((size.width - self.right).zero_height),
            ]
    ...
    def _stretch_row(
        ...
        relative_center = center.shift(self.left.zero_height)
        relative_right = right.shift(size.width.zero_height, Anchor.TOP_RIGHT)
