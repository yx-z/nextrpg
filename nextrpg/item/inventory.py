...
from nextrpg.config.item_config import (
    BaseItemKey,
    ItemCategory,
    ItemConfig,
    ItemKeyAndQuantity,
...
class Inventory(LoadFromSave):
    ...
    def __contains__(
        ...
        return all(
            self.get_item(it) >= qty for it, qty in _item_and_quantities(item)
        )
    ...
    def __add__(
        ...
        updates = {
            it: self.get_item(it) + qty
            for it, qty in _item_and_quantities(item)
        }
        res = self.items | updates
        return replace(self, items=frozendict(res))

    def get_item(self, item: BaseItemKey) -> int:
