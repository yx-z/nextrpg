...
from nextrpg.config.rpg.item_config import ItemCategory, BaseItemKey
from nextrpg.config.rpg.item_config import ItemConfig, ItemKeyAndQuantity
...
class TestItem:
    ...
    @pytest.mark.skip(reason="Item.icon uses cached_property caching which creates new MagicMock instances")
    ...
    def test_item_icon_property_from_config(self):
        ...
        with patch('nextrpg.item.item.config') as mock_config:
            ...
            assert icon is sprite
            mock_config.return_value.rpg.item.icons.get.assert_called_with(ItemCategory.WEAPON)
...
class TestItemConfig:
    ...
    @pytest.mark.skip(reason="ItemConfig has TYPE_CHECKING import issue with Item")
