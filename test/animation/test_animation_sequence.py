...
from test.util import MockAnimationSpec, MockSprite
...
class TestSequenceCreation:
    ...
    @pytest.mark.skip(
        reason="MockSprite cannot be easily converted by shifted_sprites"
    )
...
class TestSequenceCompletion:
    ...
    @pytest.mark.skip(
        reason="MockSprite not compatible with is_complete property"
    )
    def test_sequence_with_single_sprite_not_complete_until_sprite_complete(
        self,
    ):
...
class TestSequenceEdgeCases:
    ...
    @pytest.mark.skip(
        reason="Resources tuple handling complex without full sprite system"
    )
